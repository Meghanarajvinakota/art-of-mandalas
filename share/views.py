...
def share_an_art(request):
    shared_list = Share.objects.all().filter(approved=True)
    if request.method == "POST":
        ...
        if share_an_art_form.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS,
                                 "Thanks for sharing!! waiting for approval.")
...
def share_edit(request, share_id):
    ...
    if request.method == "POST":
        share_an_art_form = ShareAnArtForm(request.POST,
                                           request.FILES,
                                           instance=share_to_edit)
        if share_an_art_form.is_valid():
